...
function updatePositions() {
  ...
  var phase = [];
  for (var i = 0; i < 5; i++) {
    phase.push(Math.sin(scrollNumber + i) * 100);
  }
  for (var i = 0; i < items.length; i++) {
    // phase = Math.sin(scrollNumber + (i % 5));
    var left = items[i].basicLeft + phase[i % 5];
...
// the number of sliding pizzas is generated base on the window.innerHeight
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var n = window.innerHeight / s;
  console.log(n * cols);
